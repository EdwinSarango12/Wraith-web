    ...
    e => {
      ...
      Object.keys(rings).map(Number).sort((a, b) => a - b).forEach(ring => {
        ...
        rings[ring].forEach(cube => {
          const r = +cube.dataset.row;
          const c = +cube.dataset.col;
          const resetColor = colorMap[`${r}-${c}`] || faceColor;
          ...
          gsap.to(faces, { backgroundColor: resetColor, duration: animDuration, delay: delay + animDuration + holdTime, ease: 'power3.out' });
        });
      });
    },
    [rippleOnClick, gridSize, faceColor, colorMap, rippleColor, rippleSpeed]
  );
  ...
  return (
    <div className="relative w-1/2 max-md:w-11/12 aspect-square" style={wrapperStyle}>
      <div ref={sceneRef} className="grid w-full h-full" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => {
            const cellColor = colorMap[`${r}-${c}`] || faceColor;
            const faceStyle = { background: cellColor, border: 'var(--cube-face-border)', boxShadow: 'var(--cube-face-shadow)' };
            return (
              <div
                key={`${r}-${c}`}
              ...
              >
                ...
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'translateY(-50%) rotateX(90deg)' }} />
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'translateY(50%) rotateX(-90deg)' }} />
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'translateX(-50%) rotateY(-90deg)' }} />
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'translateX(50%) rotateY(90deg)' }} />
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'rotateY(-90deg) translateX(50%) rotateY(90deg)' }} />
                <div className="cube-face absolute inset-0 flex items-center justify-center" style={{ ...faceStyle, transform: 'rotateY(90deg) translateX(-50%) rotateY(-90deg)' }} />
              </div>
            );
          })
